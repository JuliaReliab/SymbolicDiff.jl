...
function symboliceval(f::SymbolicValue{Tx}, dvar::Symbol, env::SymbolicEnv{Tv}, cache::SymbolicCache) where {Tx,Tv}
    Tv(0)
end

function symboliceval(f::SymbolicVariable, dvar::Symbol, env::SymbolicEnv{Tv}, cache::SymbolicCache) where {Tx,Tv}
...
function _eval(::Val{:+}, f::SymbolicExpression, dvar::Symbol, env::SymbolicEnv{Tv}, cache::SymbolicCache) where Tv
...
function _eval(::Val{:exp}, f::SymbolicExpression, dvar::Symbol, env::SymbolicEnv{Tv}, cache::SymbolicCache) where Tv
    x, = [symboliceval(x, env, cache) for x = f.args]
    dx, = [symboliceval(x, dvar, env, cache) for x = f.args]
    exp(x) * dx
end

function _eval(::Val{:log}, f::SymbolicExpression, dvar::Symbol, env::SymbolicEnv{Tv}, cache::SymbolicCache) where Tv
    x, = [symboliceval(x, env, cache) for x = f.args]
    dx, = [symboliceval(x, dvar, env, cache) for x = f.args]
    dx / x
end

function _eval(::Val{:sqrt}, f::SymbolicExpression, dvar::Symbol, env::SymbolicEnv{Tv}, cache::SymbolicCache) where Tv
    x, = [symboliceval(x, env, cache) for x = f.args]
    dx, = [symboliceval(x, dvar, env, cache) for x = f.args]
    dx /(Tv(2) * sqrt(x))
end
